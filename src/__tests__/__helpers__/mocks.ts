...
export function mockTds() {
  vi.mock("@toss/tds-mobile", () => ({
    ...
    // FixedBottomCTA is itself a <button> (.d.ts: ref HTMLButtonElement) — SubmitFooter
    // relies on this shape (children = label, no nested <Button>).
    FixedBottomCTA: Object.assign(
      ({ children, onClick, disabled, ...props }: any) =>
        React.createElement("button", { onClick, disabled, "data-slot": "fixed-bottom-cta", ...props }, children),
      {
        Double: ({ children }: any) =>
